...
const UserProfile: React.FC<UserProfileProps> = ({ onSave }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/update_user/${editableUser.id}`, {
  ...
  const handlePasswordSave = async () => {
    ...
    try {
      if (!user) return;
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/update_user/${user.id}`, {
